...
The random load pattern is used for training. It produces a stream of requests at the rate of $l_i(t) \in {\{5,10,15,20\}}$ requests/second for information services $S_1$ and $S_2$ and $l_i(t) \in {\{1,2,3,4,5\}}$ for the compute service $S_3$. At every time step, the load value changes to a value drawn uniformly at random from $\{5,10,15,20\}$ for information services and from $\{1,2,3,4,5\}$ for the compute service. A time step is $5$ seconds on the testbed for Scenarios $1-3$ and one minute for Scenario $4$.
